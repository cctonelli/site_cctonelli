...
const HeroCarousel: React.FC<HeroCarouselProps> = ({ slides, t, resolveContent }) => {
  ...
  return (
    <section id="hero" className="relative h-screen bg-white dark:bg-brand-navy overflow-hidden transition-colors duration-500">
      ...
      {/* CAMADA 0: O GLOBO E A GRANDE REDE (Sempre Presente) */}
      <div className="absolute inset-0 z-0 opacity-60 dark:opacity-80 scale-110 lg:scale-100 pointer-events-none">
        <ThreeGlobe />
      </div>

      {/* CAMADA 1: CONTEÚDO (Slides ou Fallback) */}
          ...
          <div className="h-full flex items-center">
            <div className="container mx-auto px-6">
              <motion.div 
                ...
                className="max-w-4xl p-10 lg:p-16 glass rounded-[3rem] space-y-10 border border-slate-200/50 dark:border-white/10 shadow-2xl backdrop-blur-md"
              >
              ...
              <SwiperSlide key={slide.id}>
                <div className="relative h-full w-full flex items-center overflow-hidden">
                  ...
                  <div className="absolute inset-0 overflow-hidden">
                    <img 
                      src={slide.url} 
                      className="w-full h-full object-cover opacity-40 dark:opacity-30 ken-burns" 
                      alt={slide.title || ''} 
                    />
                  </div>
                  
                  {/* Gradiente de Mascaramento para legibilidade e integração com o Globo */}
                  ...
                  <div className="container mx-auto px-6 relative z-20">
                    <motion.div 
                      ...
                      className="max-w-4xl p-10 lg:p-16 glass rounded-[3rem] space-y-8 lg:space-y-10 border border-slate-200/50 dark:border-white/10 shadow-2xl backdrop-blur-sm"
                    >
                      <span className="inline-block px-5 py-2 bg-blue-600/10 border border-blue-600/20 rounded-full text-blue-600 dark:text-blue-500 text-[10px] font-black uppercase tracking-[0.4em]">
                        {resolveContent('hero_badge', t.hero_badge)}
                      </span>
