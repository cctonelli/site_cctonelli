...
const ProductPage: React.FC<ProductPageProps> = ({ language, t, resolveTranslation }) => {
  ...
  const loadData = async (targetSlug: string) => {
    setLoading(true);
    try {
      // O fetchProductBySlug já implementa o fallback híbrido (DB -> Local)
      ...
      // Fetch de variantes e blocos com fallback automático
          ...
          <section key={block.id} id="pricing" className="py-64 bg-black relative border-y border-white/5">
            <div className="container mx-auto px-6 relative z-10">
              ...
              <div className="grid lg:grid-cols-3 gap-16 max-w-7xl mx-auto">
                {variants.map((v) => (
                  <div key={v.id} className={`p-16 rounded-[4.5rem] border flex flex-col justify-between transition-all duration-700 ${v.is_most_popular ? 'bg-blue-600 border-blue-400 text-white shadow-2xl scale-105 z-10' : 'bg-slate-900/60 border-white/5 text-slate-400 hover:border-blue-600/30'}`}>
                    ...
                    <Link to={`/loja/${product.slug}/checkout?variant_id=${v.id}`} className={`w-full py-8 mt-24 rounded-3xl font-black uppercase tracking-[0.6em] text-[11px] text-center transition-all ${v.is_most_popular ? 'bg-white text-blue-600' : 'bg-blue-600 text-white'}`}>
                      ATIVAR PROTOCOLO
                    </Link>
                  </div>
                ))}
