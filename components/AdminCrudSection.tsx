...
const AdminCrudSection: React.FC<AdminCrudSectionProps> = ({ 
  ...
  const loadData = useCallback(async () => {
    ...
    if (error) {
      setMessage({ text: 'Erro ao carregar: ' + error.message, type: 'error' });
    } else {
      setItems(data || []);
    }
    setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage({ text: 'Configuração JSON inválida.', type: 'error' });
      setLoading(false);
      return;
    }
    ...
    if (error) {
      setMessage({ text: 'Falha ao salvar: ' + error.message, type: 'error' });
    } else {
  ...
  const handleToggle = async (id: any, column: string, val: any) => {
    const newVal = !(val === true || val === 'true');
    await supabase.from(tableName).update({ [column]: newVal }).eq(idColumn, id);
    loadData();
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4">
          {filteredItems.map(item => (
            <div key={item[idColumn]} className="bg-slate-900/20 border border-white/5 p-6 rounded-[2rem] flex items-center justify-between hover:border-blue-500/30 transition-all group">
              ...
              <div className="flex gap-2">
                <button onClick={() => handleToggle(item[idColumn], item.is_active !== undefined ? 'is_active' : 'approved', item.is_active ?? item.approved)} className={`p-3 rounded-xl text-[8px] font-bold uppercase transition-all ${ (item.is_active === true || item.is_active === 'true' || item.approved === true || item.approved === 'true') ? 'bg-blue-500/10 text-blue-500' : 'bg-red-500/10 text-red-500'}`}>
                  {(item.is_active === true || item.is_active === 'true' || item.approved === true || item.approved === 'true') ? 'Ativo' : 'Off'}
                </button>
